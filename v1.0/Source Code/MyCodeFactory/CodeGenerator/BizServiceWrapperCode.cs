...
{
    public class BizServiceWrapperCode
    {
        private Type _itemType = null;

        public BizServiceWrapperCode(Type type)
        {
            this._itemType = type;
        }
        ...
        private void WriteMethods(Type type, StringWriter writer)
        {
            ...
            foreach (MethodInfo info in methods)
            {
                if(info.ReturnType == typeof(void))
                {
                    ...
                    writer.WriteLine("\t\t\tServiceBuilder.Get{0}(token).{1}({2});", type.Name.Substring(4), info.Name, this.GetParaNameList(info));
                ...
                else
                {
                    writer.WriteLine("\t\tpublic static {0} {1}({2})", info.ReturnType, info.Name, this.GetParas(info));
                    writer.WriteLine("\t\t{");
                    writer.WriteLine("\t\t\treturn ServiceBuilder.Get{0}(token).{1}({2});", type.Name.Substring(4),
        ...
        private string GetParas(MethodInfo method)
        {
            ...
            builder.AppendFormat(" Cheke.SecurityToken token");
