...
{
    public class BizServiceBuilderCode
    {
        private Assembly _assembly = null;

        public BizServiceBuilderCode(Assembly assembly)
        {
        ...
        private void WriteContent(StringWriter writer)
        {
            ...
            foreach (Type item in types)
            {
                ...
                {
                    writer.WriteLine("\t\tinternal static {0} Get{1}(Cheke.SecurityToken token)", item.FullName, item.Name.Substring(4));
                    writer.WriteLine("\t\t{");
                    writer.WriteLine("\t\t\treturn ServiceFactory.Get{0}(token);", item.Name.Substring(4));
                    writer.WriteLine("\t\t}");
                    writer.WriteLine();
                }
            }
